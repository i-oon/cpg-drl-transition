...
The discrete spike is unambiguous: max |joint velocity| reaches **16.4 rad/s** at the instant of switch — **×8 larger than Residual-α 4D (2.0 rad/s)**. Linear ramp and Smoothstep sit between the two. The forward velocity panel shows discrete's characteristic velocity dip and partial reversal immediately after the switch, while all blended methods maintain positive velocity throughout. Note that linear ramp's spike (1.78 rad/s) is slightly lower than smoothstep's (2.57 rad/s) at the first instant — because linear starts ramping earlier and at a uniform rate — but both methods produce larger sustained jerk than the learned residual.

**2×2 ablation (output space × action dimension):**

![Transition zoom — ablation](logs/phase2/transition_zoom_ablation.png)

Blue = α-space output (Δα); Orange = q-space output (Δq). Solid = 4D; Dashed = 12D. Res-α 4D (Ours, blue solid) achieves the lowest max |joint velocity| (2.0 rad/s). Res-α 12D (blue dashed) is the degenerate variant — the policy collapses to near-zero corrections, producing a spike (3.4 rad/s) that exceeds even Res-q 4D (3.5 rad/s). Res-q 12D (orange dashed) scores comparably to Res-α 4D on the instant spike (2.1 rad/s) but has higher jerk_TRANS across the full window (9750 vs 7965). The 4D α-space variant is optimal on both metrics.
...
- [x] Discrete spike quantified: 16.4 rad/s max |jv| at switch vs 2.0 rad/s (Residual-α 4D), ×8 ratio
