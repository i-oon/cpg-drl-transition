"""
Phase 1 – Task 2: Train W_trot via PIBB.
...
parser = argparse.ArgumentParser(description="Train W_trot (Phase 1 PIBB)")
AppLauncher.add_app_launcher_args(parser)
parser.add_argument("--config", type=str, default="configs/phase1_trot.yaml")
...
logger.info("Starting PIBB training — trot gait (direct encoding)")
trainer = PIBBTrainer(env, config, log_dir=log_dir)
W_trot = trainer.train()

logger.info("W_trot saved to %s", config["output"]["weights_path"])
